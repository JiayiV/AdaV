...
class LlavaLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def generate(
        ...
        **kwargs,
        ...
        """
        with open(os.path.join(os.getcwd(),"scripts/adav_config.json")) as f:
            data = json.load(f)
            f.close()

        method = data['method']
        overall_tokens = data['overall_tokens']
        ...
        # ====================================================
        #    modification version 2: [CLS] attn.
        # ====================================================
        if method == 'mix':
            N_img = cls_attn.shape[0]
            base = int(overall_tokens / 2)
            k = int(base / N_img)
            B, N, C = inputs_embeds.shape
            forward_tokens = inputs_embeds[:, 35:, :]
            pre_tokens = inputs_embeds[:,:35,:]
            ...
            # for the [CLS] attn.
            _, indices_CLS = torch.topk(cls_attn, k=k*2, dim=-1) # B, k
            ...
            indices = torch.sort(indices_CLS, dim=1)[0] # (B, T)
        ...
        if method == "adav":
            ...
            if "vicuna" in model_name or "v1.5" in model_name:
                pre_n_token = 35
            elif "34b" in model_name:
                pre_n_token = 10
            else:
                print("ERROR: model name not supported")
                exit()
            ...
            budget_thres = 0.25
            B, N, C = inputs_embeds.shape
            forward_tokens = inputs_embeds[:, pre_n_token:, :]
            pre_tokens = inputs_embeds[:,:pre_n_token,:]
            image_tokens = forward_tokens[:, :(576*N_img), :]
            text_tokens = forward_tokens[:, (576*N_img):, :]
        
            simi = torch.matmul(F.normalize(text_tokens, p=2, dim=-1), 
                                F.normalize(image_tokens, p=2, dim=-1).permute(0,2,1))
            # simi = (simi*100).softmax(-1).max(1)[0] # B, Ni # the standard setting
            simi = (simi*100).softmax(-1).max(1)[0]
